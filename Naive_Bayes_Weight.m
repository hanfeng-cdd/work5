function [test_label,h]=Naive_Bayes_Weight(data_type,train_data,test_data,m,D)
...
p_label=sum(D(x_label==1));
n_label=sum(D(x_label==-1));
feature={};
...
for j=1:length(discrete_num)
    ...
    D_discrete=repmat(D,[1,value_length]);
    
    flag=repmat(column_x,[1,value_length])-repmat(feature{index}.value',[train_num,1]);
    flag_1=flag;
    flag_1(x_label==-1,:)=1;
    feature{index}.probability_p=(sum( D_discrete.*(flag_1==0))+m)/(p_label+value_length*m);
    flag_0=flag;
    flag_0(x_label==1,:)=1;
    feature{index}.probability_n=(sum( D_discrete.*(flag_0==0))+m)/(n_label+value_length*m);
end
...
%     feature{index}.nstd=std(column_x(x_label==-1));
     ...
     feature{index}.pmean = (sum(column_x(x_label==1).*D(x_label==1)))/sum(D(x_label==1));
     feature{index}.pstd =sqrt((sum(power(column_x(x_label==1),2).*D(x_label==1)))/sum(D(x_label==1))-power(feature{index}.pmean,2));
     feature{index}.nmean = (sum(column_x(x_label==-1).*D(x_label==-1)))/sum(D(x_label==-1));
     feature{index}.nstd =sqrt((sum(power(column_x(x_label==-1),2).*D(x_label==-1)))/sum(D(x_label==-1))-power(feature{index}.nmean,2));
    
    
end

prior_p=(sum(D(x_label==1))+m)/(1+2*m);
prior_n=(sum(D(x_label==-1))+m)/(1+2*m);
